...
echo "查看日志: pm2 logs order-service"
EOF
...
pm2 stop order-service 2>/dev/null || true
pm2 delete order-service 2>/dev/null || true
...
create_package() {
    ...
    log_info "订单服务打包完成 (大小: $file_size)"
}
